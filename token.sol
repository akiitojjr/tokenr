...
contract ROFLcoptertoken is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   ROFLcoptertokenDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ROFLcoptertoken", "RCT") {
        marketingWallet = payable(0x982B848e1BCe39Ef059050A43ef51c4e25f8ddB5);
        devWallet = payable(0x982B848e1BCe39Ef059050A43ef51c4e25f8ddB5);
        ...
        dividendTracker = new ROFLcoptertokenDividendTracker(
            payable(this),
            router,
            0x6982508145454Ce325dDbE47a25d4ec3d2311933, // pepe
            "ROFLcoptertokenTRACKER",
            "RCTTRACKER"
        );
        ...
        uint256 totalTokenSupply = (69_420_000_000_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 347100000000; // 0.005%
        swapTokensAtAmount = totalTokenSupply / 347100000000; // 0.05%;
...
contract ROFLcoptertokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    ROFLcoptertoken public ROFLcoptertokenContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ROFLcoptertokenContract = ROFLcoptertoken(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ROFLcoptertoken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ROFLcoptertoken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ROFLcoptertoken contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ROFLcoptertokenContract);

            uint256 prevBalance = ROFLcoptertokenContract.balanceOf(address(this));
            ...
            {
                uint256 received = ROFLcoptertokenContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ROFLcoptertokenContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
